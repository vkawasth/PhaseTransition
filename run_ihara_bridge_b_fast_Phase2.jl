...
NOTE : 
The original code hardcoded the trinion eigenvalues (λ₁=1.7898, λ₂=−0.8021)
for the b1=1 cylinder case too. For Q_7L the scalar monodromy should use
the cylinder's single eigenvalue λ=1.5731. The accumulation was computing:
Phi_KS = 1.7898^245 × (−0.8021)^239 ≈ −1.116×10³⁹   ← old (wrong)
Phi_KS = 1.5731^245 × (1/1.5731)^239 = 1.5731^6 ≈ 15.15   ← new (correct)
"""
...
function h1_matrix(Δφ::Float64, b1::Int, ihara_lam1::Float64, ihara_lam2::Float64)
    # Algebraic H1 monodromy from B_Ihara|_H1
    # eigenvalues are graph-specific: passed in from the ihara_lam1/lam2 block below
    tr_B  = ihara_lam1 + ihara_lam2
    det_B = ihara_lam1 * ihara_lam2
    if b1 == 1
        # Cylinder: scalar monodromy.
        # Forward crossing (Δφ>0): multiply by λ (the single non-trivial eigenvalue)
        # Reverse crossing (Δφ<0): multiply by 1/λ (inverse)
        # Φ_KS = λ^n+ × (1/λ)^n- = λ^(n+ - n-) = λ^w_net
        return reshape([Δφ > 0 ? ihara_lam1 : (1.0 / ihara_lam1)], 1, 1)
    else
        # Trinion: companion matrix of B_Ihara|_H1
        # Forward crossing (Δφ>0): M = B|_H1
        # Reverse crossing (Δφ<0): M = (B|_H1)^{-1}
        M_fwd = [0.0  det_B; 1.0  tr_B]
        M_inv = inv(M_fwd)
        return Δφ >= 0 ? M_fwd : M_inv
    end
end

# ── Ihara H1 eigenvalues (needed by h1_matrix) ────────────────────────────────
# Defined before the snapshot loop so h1_matrix can use them.
# Q_7P / Q_8 (trinion, b1=2): λ1=1.7898, λ2=-0.8021
# Q_7L / Q_6 (cylinder, b1=1): λ1=1.5731, λ2=0.0
_ihara_lam1 = (b1 == 2) ? 1.7898 : 1.5731
_ihara_lam2 = (b1 == 2) ? -0.8021 : 0.0
...
for i in wall_steps
    ...
    M  = h1_matrix(Δφ, b1, _ihara_lam1, _ihara_lam2)
...
if isempty(wc_data)
    ...
    ih_a1       = -ihara_tr                  # -0.9877  (coeff of u  in ζ⁻¹)
    # ih_a2: coeff of u² in det(I−u·M_fwd).
    # M_fwd = [[0, ihara_det],[1, ihara_tr]] → det(M_fwd) = -ihara_det = +1.4356
    # det(I−u·M_fwd) = 1 − tr·u + det(M_fwd)·u² = 1 − ihara_tr·u − ihara_det·u²
    # ζ⁻¹|_H1 written as 1 − ihara_tr·u + ihara_det·u² uses the OTHER sign.
    # Use ih_a2 = -ihara_det so da2 compares like-for-like with det(M_fwd).
    ih_a2       = -ihara_det                 # +1.4356  (= det(M_fwd), coeff of u²)
    ...
    elseif b1 == 1
        ...
        # Phi_KS = λ^w_net.
        # Per-loop: |Phi_KS|^(1/|w|) = λ^(w/|w|).
        # For w>0 that's λ; for w<0 that's 1/λ — so invert for negative winding.
        if w_net != 0
            raw = abs(Phi_KS)^(1.0/abs(w_net))
            loop_scalar = w_net > 0 ? raw : 1.0/raw
        else
            loop_scalar = abs(Phi_KS)
        end
        da = abs(loop_scalar - ihara_lam1)
        @printf("Scalar Φ_KS (accumulated) = %.6f  (= λ^%d)\n", Phi_KS, w_net)
        @printf("Per-loop Φ_KS^(1/|w|)%s   = %.6f  (target λ1=%.4f)  Δ=%.4f\n",
                w_net < 0 ? " inverted" : "         ", loop_scalar, ihara_lam1, da)
        if da < 0.05
    ...
    open(resfile, "w") do io
        ...
        println(io, "")
        # ── Winding counts ────────────────────────────────────────────────
        n_fwd_w = sum(1 for r in wc_data if r.delta_phi > 0)
        n_inv_w = sum(1 for r in wc_data if r.delta_phi < 0)
        w_net_w = n_fwd_w - n_inv_w
        println(io, "Wall crossing counts:")
        println(io, "  n+ (forward, Δφ>0) = $n_fwd_w")
        println(io, "  n- (backward, Δφ<0) = $n_inv_w")
        println(io, "  w = n+ - n- = $w_net_w  (net winding number)")
        println(io, "")
        # ── Accumulated Φ_KS ─────────────────────────────────────────────
        if b1 == 1
            println(io, "Φ_KS (accumulated) = λ^w = $(round(ihara_lam1,digits=6))^$w_net_w = $(round(Phi_KS,digits=6))")
            println(io, "  (w=$w_net_w: $(w_net_w>0 ? "net forward" : w_net_w<0 ? "net backward" : "no net") winding)")
            println(io, "det(I-u·Φ_KS)      = 1 - $(round(Phi_KS,digits=6))·u")
            println(io, "ζ_Ihara^{-1}|_{H1} = 1 - $(round(ihara_lam1,digits=6))·u")
            println(io, "")
            # Per-loop scalar
            raw_s  = w_net_w != 0 ? abs(Phi_KS)^(1.0/abs(w_net_w)) : abs(Phi_KS)
            loop_s = (w_net_w != 0 && w_net_w < 0) ? 1.0/raw_s : raw_s
            da_s   = abs(loop_s - ihara_lam1)
            println(io, "Per-loop monodromy:")
            println(io, "  Φ_KS^(1/|w|)$(w_net_w<0 ? " inverted" : "        ") = $(round(loop_s,digits=6))  ($(w_net_w<0 ? "1/" : "")$(round(abs(Phi_KS),digits=6))^(1/$(abs(w_net_w))))")
            println(io, "  λ_Ihara    = $(round(ihara_lam1,digits=6))")
            println(io, "  Δ          = $(round(da_s,digits=6))")
            println(io, "  det(I-u·Φ_loop) = 1 - $(round(loop_s,digits=6))·u")
            println(io, "  ζ_Ihara^{-1}|_{H1} = 1 - $(round(ihara_lam1,digits=6))·u")
            println(io, "")
            if da_s < 0.05
                println(io, "✓✓✓  BRIDGE B CONFIRMED (cylinder, per-loop scalar, Δ=$(round(da_s,digits=6)))")
            else
                println(io, "~    Bridge B not confirmed  Δ=$(round(da_s,digits=6))")
            end
        else
            println(io, "Φ_KS (accumulated):")
            @printf(io, "  [[%+.6f  %+.6f]\n   [%+.6f  %+.6f]]\n",
                    Phi_KS[1,1], Phi_KS[1,2], Phi_KS[2,1], Phi_KS[2,2])
            @printf(io, "  tr(Φ_KS)  = %.6f\n", tr_Phi)
            @printf(io, "  det(Φ_KS) = %.6f\n", det_Phi)
            ...
            println(io, "")
            # Per-loop: Φ_loop = M_fwd^1 when Φ_KS = M_fwd^w
            if w_net_w != 0
                M_fwd_w = [0.0 ihara_det; 1.0 ihara_tr]
                Phi_w   = M_fwd_w^w_net_w
                tr_w    = tr(Phi_w); det_w = det(Phi_w)
                match_w = abs(tr_Phi - tr_w) < 0.01 && abs(det_Phi - det_w) < 0.05
                println(io, "Per-loop monodromy (Φ_KS = M_fwd^$w_net_w ?):")
                @printf(io, "  Expected M_fwd^%d: tr=%.6f  det=%.6f\n", w_net_w, tr_w, det_w)
                @printf(io, "  Computed  Φ_KS:   tr=%.6f  det=%.6f\n", tr_Phi, det_Phi)
                if match_w
                    println(io, "  ✓  Φ_KS = M_fwd^$w_net_w confirmed")
                    println(io, "  Per-loop Φ_loop = M_fwd:")
                    @printf(io, "    [[%+.6f  %+.6f]\n     [%+.6f  %+.6f]]\n",
                            M_fwd_w[1,1], M_fwd_w[1,2], M_fwd_w[2,1], M_fwd_w[2,2])
                    @printf(io, "    tr = %.6f  det = %.6f\n", tr(M_fwd_w), det(M_fwd_w))
                    da1 = abs(-tr(M_fwd_w) - ih_a1)
                    da2 = abs(det(M_fwd_w) - ih_a2)   # det(M_fwd)=+1.4356, ih_a2=+1.4356 ✓
                    println(io, "  det(I-u·Φ_loop) = 1 - $(round(tr(M_fwd_w),digits=6))·u + $(round(det(M_fwd_w),digits=6))·u²")
                    println(io, "  ζ_Ihara^{-1}|_{H1} = 1 - $(round(ihara_tr,digits=6))·u + $(round(-ihara_det,digits=6))·u²")
                    println(io, "")
                    if da1 < 0.01 && da2 < 0.01
                        println(io, "✓✓✓  BRIDGE B CONFIRMED (trinion, per-loop monodromy)")
                    else
                        @printf(io, "~    Per-loop mismatch: Δ(u)=%.4f  Δ(u²)=%.4f\n", da1, da2)
                    end
                else
                    println(io, "  ~  Φ_KS ≠ M_fwd^$w_net_w — see console for details")
                end
            else
                println(io, "Net winding w=0 — per-loop monodromy undefined.")
            end
        end
    end
