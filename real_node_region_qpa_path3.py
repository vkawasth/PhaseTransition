...
def generate_consolidated_qpa(node_path, edge_path):
    ...
    print("\n--- Phase 4: Build Length-2 Generators ---")

    length2_generators = []
    for (i, j, k) in permutations(TARGET_REGIONS, 3):
        if (i, j) in consolidated_flows and (j, k) in consolidated_flows:
            g_name = f"g_{i}_{j}_{k}"
            length2_generators.append((i, k, g_name))
            arrow_defs.append(f'["{i}", "{k}", "{g_name}"]')

    print(f"[+] Created {len(length2_generators)} length-2 generators.")
    ...
    # -----------------------------
    # (A) Define length-2 paths
    # -----------------------------
    print("\n[+] Defining f*f = g")

    for (i, j, k) in permutations(TARGET_REGIONS, 3):
        if (i, j) in consolidated_flows and (j, k) in consolidated_flows:
            rel = f"f_{i}_{j}*f_{j}_{k} - g_{i}_{j}_{k}"
            rels.append(rel)
            print(f"Relation: {rel}")

    # -----------------------------
    # (B) Collapse length-3 paths
    # -----------------------------
    print("\n[+] Collapsing length-3 paths")

    for (i, j, k, l) in permutations(TARGET_REGIONS, 4):
        if (
            (i, j) in consolidated_flows and
            (j, k) in consolidated_flows and
            (k, l) in consolidated_flows
        ):
            wij = consolidated_flows[(i, j)]
            wjk = consolidated_flows[(j, k)]
            wkl = consolidated_flows[(k, l)]

            lam = (wij * wjk * wkl) % 101

            rel1 = f"g_{i}_{j}_{k}*f_{k}_{l} - {lam}*f_{i}_{l}"
            rel2 = f"f_{i}_{j}*g_{j}_{k}_{l} - {lam}*f_{i}_{l}"

            rels.append(rel1)
            rels.append(rel2)

            print(f"Relation: {rel1}")
            print(f"Relation: {rel2}")
    ...
    with open("brain_complex_quiver_GE3.g", "w") as f:
        f.write("\n".join(gap_code))

    print("\n✅ File saved: brain_complex_quiver_GE3.g")
