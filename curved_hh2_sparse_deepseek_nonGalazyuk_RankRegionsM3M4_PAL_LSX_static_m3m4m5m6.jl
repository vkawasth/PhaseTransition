...
# ------------------------------------------------------------
nodes = [:CA1sp, :HPF, :BLA, :sAMY, :HY, :LA, :PAL, :LSX]   # add :PAL later
#nodes = [:CA1sp, :HPF, :BLA, :sAMY, :HY, :LA, :PAL, :LSX, :RHP]

# Following are generated using OGB Stanford BALBc voxel graph for Quiver paths using 
# real_node_region_qpa_PAL.py passing nodes with tagged regions (node_regions_clean.csv) and BALBc_no1_raw edges files.
# Following scalar multipliers inlude edge cross section and curvature into weight calculations
# Field of prime dows not allow reverse hironaka path attribution to actual edges/nodes on graph as modulo arithmetic
# looses that level of resoultion in contravariant functor.
# python real_node_region_qpa_PAL_LSX.py node_regions_clean.csv ../../BALBc_no1_raw/BALBc-no1_iso3um_stitched_segmentation_bulge_size_3.0_edges.csv
# That prograp generated GAP QPA code, extract relations from that code. GAP QPA can't do non associative algebra.

relations_str = """
# Compositional Relations
f_CA1sp_HPF*f_HPF_BLA - 104848401.13425562*f_CA1sp_BLA
f_CA1sp_HPF*f_HPF_sAMY - 1170812.3569494174*f_CA1sp_sAMY
f_CA1sp_HPF*f_HPF_LSX - 2174344976.588707*f_CA1sp_LSX
...
f_CA1sp_sAMY*f_sAMY_PAL - 22727900.5977732*f_CA1sp_PAL
...
f_BLA_sAMY*f_sAMY_PAL - 1482307863.9205763*f_BLA_PAL
...
f_HPF_sAMY*f_sAMY_PAL - 144741906.61248878*f_HPF_PAL
...
f_sAMY_HPF*f_HPF_LSX - 39226930.78104828*f_sAMY_LSX
...
f_PAL_sAMY*f_sAMY_BLA - 3116565334.9096637*f_PAL_BLA
f_PAL_sAMY*f_sAMY_HY - 5206.33905724803*f_PAL_HY
f_PAL_sAMY*f_sAMY_HPF - 243820551.1305996*f_PAL_HPF
f_PAL_sAMY*f_sAMY_LA - 386159744.07654345*f_PAL_LA
f_LSX_HPF*f_HPF_CA1sp - 2012159659.5517678*f_LSX_CA1sp
f_LSX_HPF*f_HPF_BLA - 5255473.831781052*f_LSX_BLA
f_LSX_HPF*f_HPF_sAMY - 42874328.861522146*f_LSX_sAMY

# Identity / Loop Closures
...
#=  LSX only for reference to see how quiver path algebra changes with added resolutions
relations_str = """ 
f_CA1sp_HPF*f_HPF_BLA - 104848401.13425562*f_CA1sp_BLA
f_CA1sp_HPF*f_HPF_sAMY - 1170812.3569494174*f_CA1sp_sAMY
f_CA1sp_HPF*f_HPF_LSX - 2174344976.588707*f_CA1sp_LSX
...
sheaf = build_sheaf(basis, mult_dict, m3_element, m4_obs,
    ["sAMY","HPF","BLA","CA1sp","HY","LA", "PAL", "LSX"])
...
regions = ["sAMY", "HPF", "BLA", "CA1sp", "HY", "LA", "PAL", "LSX"]
...
# Include m3, m4, m5, m6 into dynamics (static) not running differential equation for m3om4 cross effects etc.
function compute_full_region_scores(m3, m4, m5, m6; α=1.0, β=2.0, γ=3.0, δ=4.0)
    score = Dict{Symbol, Float64}()
    for (triple, d) in m3
        w = norm_dict(d)
        for x in triple
            r = symbol_to_region(x) |> Symbol
            score[r] = get(score, r, 0.0) + α * w
        end
    end
    for (quad, d) in m4
        w = norm_dict(d)
    ...
    for (sext, d) in m6
        ...
        for x in sext
            r = symbol_to_region(x) |> Symbol
            score[r] = get(score, r, 0.0) + δ * w
        end
    end
    return score
end

region_scores_full = compute_full_region_scores(m3_element, m4_corrected, m5_global, m6_global)
function region_scores_to_node_scores(region_scores, node_regions)
    node_scores = Dict{Int, Float64}()
    for (nid, reglist) in node_regions
        s = 0.0
        for r in reglist
            s += get(region_scores, Symbol(r), 0.0)
        end
        node_scores[nid] = s
    end
    return node_scores
...
node_score_m3 = region_scores_to_node_scores(compute_full_region_scores(m3_element, Dict(), Dict(), Dict()), node_regions)
node_score_m4 = region_scores_to_node_scores(compute_full_region_scores(Dict(), m4_corrected, Dict(), Dict()), node_regions)
node_score_m5 = region_scores_to_node_scores(compute_full_region_scores(Dict(), Dict(), m5_global, Dict()), node_regions)
node_score_m6 = region_scores_to_node_scores(compute_full_region_scores(Dict(), Dict(), Dict(), m6_global), node_regions)
...
function fast_state_to_scores(state, nodes_df, node_regions)
    ...
    local_region_names = [:sAMY, :HPF, :BLA, :CA1sp, :HY, :LA, :PAL, :LSX]
...
# ------------------------------------------------------------
region_names = ["sAMY", "HPF", "BLA", "CA1sp", "HY", "LA", "PAL", "LSX"]
...
=#
println("Writing perturbed VTU files with static m3..m6 fields...")
for t in 1:nt
    node_activity = state_to_node_scores(perturbed_traj[t], node_regions)
    activity_log = log10.(abs.(node_activity) .+ 1.0)
    
    filename = "allm_perturbed_t$(lpad(t,3,"0"))"
    vtk_grid(filename, points, cells; append=true) do vtk
        # time‑varying activity
        vtk["activity"] = Float32.(activity_log)
        
        # static structural fields (same for all t)
        vtk["m3_static"] = Float32.([get(node_score_m3, id, 0.0) for id in nodes_df.id])
        vtk["m4_static"] = Float32.([get(node_score_m4, id, 0.0) for id in nodes_df.id])
        vtk["m5_static"] = Float32.([get(node_score_m5, id, 0.0) for id in nodes_df.id])
        vtk["m6_static"] = Float32.([get(node_score_m6, id, 0.0) for id in nodes_df.id])
        
        # optional: also store region name as integer label
        region_id = [findfirst(==(first(node_regions[id])), region_names) for id in nodes_df.id]
        vtk["region_id"] = region_id
    end
end
