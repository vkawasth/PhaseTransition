###############################################################
# test_klein_spectral_relation.jl
#
...
#
# Specifically checks:
#   1. ||T_N||² - q_N  ∝  |Klein constraint|   (linear relation)
...
#
# Requires outputs from run_iharaSingV2_MonoTwist.jl:
#   - plucker_trajectory.json  (has klein_constraint field from new Python)
#   - ihara_poles.json         (from IharaAssociahedronBridgeV2.jl rewrite)
###############################################################
...
function load_and_align(folder::String)
    """
    Load plucker_trajectory.json and ihara_poles.json and align
    them by snapshot index.

    Returns a NamedTuple with per-snapshot aggregated values:
        - klein_constraint : |K| per snapshot (from plucker history)
        - spectral_radius  : max |λ| per snapshot (from poles)
        - q_val            : mean degree per snapshot (ramanujan_bound²)
        - ramanujan_bound  : sqrtq per snapshot
        - re_mean          : mean Re(λ) per snapshot
        - im_vals          : all Im(λ) per snapshot (for zero comparison)
        - snapshot_indices : aligned snapshot numbers
    """
    plucker_file = joinpath(folder, "plucker_trajectory.json")
    poles_file   = joinpath(folder, "ihara_poles.json")

    if !isfile(plucker_file)
        error("plucker_trajectory.json not found. Run Python simulation first.")
    end
    if !isfile(poles_file)
        error("ihara_poles.json not found. Run IharaAssociahedronBridgeV2 first.")
    end

    plucker = JSON3.read(read(plucker_file, String))
    poles   = JSON3.read(read(poles_file, String))

    # ── Klein constraint: one value per plucker history event ──────────────
    # plucker_trajectory.json has q12..q34 Plücker coordinates.
    # Klein constraint K = |q12*q34 - q13*q24 + q14*q23| (Plücker relation)
    # If the file has a pre-computed "klein_constraint" field use it,
    # otherwise compute from the raw coordinates.
    plucker_steps = Int.(plucker["steps"])

    if haskey(plucker, "klein_constraint")
        klein_raw = Float64.(plucker["klein_constraint"])
    else
        # Compute from Plücker coordinates
        q12 = Float64.(plucker["q12"])
        q13 = Float64.(plucker["q13"])
        q14 = Float64.(plucker["q14"])
        q23 = Float64.(plucker["q23"])
        q24 = Float64.(plucker["q24"])
        q34 = Float64.(plucker["q34"])
        klein_raw = abs.(q12 .* q34 .- q13 .* q24 .+ q14 .* q23)
    end

    # ── Poles: one entry per (snapshot × eigenvalue) ───────────────────────
    # Group poles by snapshot index
    pole_array = poles["poles"]
    n_snapshots = Int(poles["n_snapshots"])
    n_per_snap  = Int(poles["n_poles_per_snap"])

    # Aggregate per snapshot
    snap_spectral_radius = zeros(n_snapshots)
    snap_q               = zeros(n_snapshots)
    snap_ramanujan       = zeros(n_snapshots)
    snap_re_mean         = zeros(n_snapshots)
    snap_im_vals         = [Float64[] for _ in 1:n_snapshots]

    for pole in pole_array
        s  = Int(pole["snapshot"])   # 1-based
        r  = Float64(pole["radius"])
        rb = Float64(pole["ramanujan_bound"])
        re = Float64(pole["re"])
        im = Float64(pole["im"])

        snap_spectral_radius[s] = max(snap_spectral_radius[s], r)
        snap_q[s]               = rb^2          # q = ramanujan_bound²
        snap_ramanujan[s]       = rb
    ...
    # plucker history events may not align 1:1 with pole snapshots.
    # Strategy: for each snapshot i, find the nearest plucker history event.
    # (They should be close since both are triggered by the same A∞ recompute.)
    ...
    for s in 1:n_snapshots
        ...
        # Find nearest plucker history step to snapshot s
        diffs = abs.(plucker_steps .- s)
        nearest_idx = argmin(diffs)
        snap_klein[s] = abs(klein_raw[nearest_idx])
    end

    return (
        klein_constraint = snap_klein,
        spectral_radius  = snap_spectral_radius,
        q_val            = snap_q,
        ramanujan_bound  = snap_ramanujan,
...
function test_lyapunov_behavior(folder::String = ".")
    ...
    # ── Test 2: conservation law ‖T‖² + K = q ─────────────────────
    conserved = ρ² .+ K
    corr = cor(conserved, q)
    @printf("\nConservation law ‖T‖² + K vs q:\n")
    @printf("  Correlation: %.6f  (want = 1.0)\n", corr)
    @printf("  Mean |conserved - q|: %.6f\n", mean(abs.(conserved .- q)))
    @printf("  Std  |conserved - q|: %.6f\n", std(abs.(conserved .- q)))

    if corr > 0.99
        println("✓ Conservation law holds: ‖T‖² + K = q exactly")
        println("  This confirms the Pythagorean decomposition of Λ²(ℝ⁴)")
    elseif corr > 0.9
        println("~ Conservation law holds approximately")
        println("  Residual may reflect proxy error in K computation")
    else
        println("✗ Conservation law not confirmed")
        println("  K proxy may need replacement with direct Klein quadric value")
    end
    ...
    # Panel 3: Conservation law ‖T‖² + K vs q
    ax3 = Axis(fig[2,1],
        title  = "C — Conservation law ‖T‖² + K = q",
        xlabel = "Snapshot index",
        ylabel = "Value")
    lines!(ax3, ids, ρ², color=:steelblue, linewidth=1.5, label="‖T‖²")
    lines!(ax3, ids, K,  color=:darkorange, linewidth=1.5, label="|K|")
    lines!(ax3, ids, conserved, color=:purple, linewidth=2,
           label="‖T‖² + K")
    q_finite = filter(isfinite, q)
    if !isempty(q_finite)
        hlines!(ax3, [mean(q_finite)], color=:red, linestyle=:dash,
                linewidth=2, label=@sprintf("mean q = %.3f", mean(q_finite)))
    end
    axislegend(ax3, position=:rt)
    text!(ax3, n * 0.05, maximum(filter(isfinite, conserved)) * 0.9,
          text=@sprintf("cor(‖T‖²+K, q) = %.4f", corr),
          color = corr > 0.99 ? :darkgreen : (corr > 0.9 ? :darkorange : :red),
          fontsize=13)
...
function test_klein_spectral_relation(folder::String = ".")
    ...
    rb_valid = d.ramanujan_bound[valid]
    ...
    # ── Ramanujan check: how many snapshots satisfy ||T|| ≤ sqrtq? ────────────
    ramanujan_ok = sum(d.spectral_radius[valid] .<= rb_valid .+ 1e-6)
    ramanujan_pct = 100 * ramanujan_ok / length(valid)

    # ── Running bound: does spectral_radius / sqrtq converge to 1? ────────────
    running_ratio = d.spectral_radius ./ max.(d.ramanujan_bound, 1e-10)
    ...
    @printf("   Snapshots satisfying ||T|| ≤ sqrtq: %d / %d (%.1f%%)\n",
            ramanujan_ok, length(valid), ramanujan_pct)
    if ramanujan_pct > 80
        println("   ✓ Ramanujan bound holds for most snapshots")
    else
        println("   ✗ Ramanujan bound violated frequently — check graph construction")
    end
...
" * "="^60)
    ...
    if isempty(ratios)
        println("  No valid consecutive pairs — check stable zone start")
        return nothing
    end
...
function find_klein_crossing(folder::String = ".")
    ...
    # ── Spectral radius and q per snapshot from poles ─────────────────────
    snap_rho = zeros(n)
    snap_q   = zeros(n)
    for pole in poles["poles"]
        s  = Int(pole["snapshot"])
...
if true  # run whether called directly or via include()
    ...
    println("="^60)
    if lya !== nothing
        @printf("  P1 Lyapunov:        λ=%.4f  K↓=%.0f%%  cor=%.3f  %s\n",
                lya.lyapunov_rate, 100*lya.pct_decreasing, lya.conservation_cor,
    ...
    if lax_comp !== nothing
        @printf("  P4b Lax ratio:      %.4f  %s\n",
                lax_comp.mean_ratio,
                lax_comp.mean_ratio<0.1 ? "✓ Lax dominant" :
                lax_comp.mean_ratio<0.5 ? "~ mixed" : "✗")
    end
    if obs_def !== nothing
